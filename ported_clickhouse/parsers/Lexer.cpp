...
#include "ported_clickhouse/base/defines.h"
#include "ported_clickhouse/common/StringUtils.h"
#include "ported_clickhouse/common/UTF8Helpers.h"
...
#include "ported_clickhouse/parsers/LexerStandalone.h"
#include "ported_clickhouse/parsers/Lexer.h"
        ...
        default:
            if (*pos == '$')
            {
                ...
                if (tag_end != end)
                {
                    ...
                    if (is_valid_name)
                    {
                        std::string_view haystack(tag_end + 1, static_cast<size_t>(end - (tag_end + 1)));
                        std::string_view needle(pos, heredoc_size);
                        size_t heredoc_end_position = haystack.find(needle);
                        if (heredoc_end_position != std::string_view::npos)
                        {
