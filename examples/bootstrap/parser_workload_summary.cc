...
int main(int argc, char ** argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            std::cout << "usage: parser_workload_summary [--query <sql>] [--query-file <path>]" << std::endl;
            return 0;
        }
        else
        {
    ...
    {
        std::cerr << "classification=parse_failed" << std::endl;
        return 1;
    }

    if (!pos->isEnd())
    {
        std::cerr << "classification=trailing_tokens" << std::endl;
        return 2;
    }
    ...
    {
        std::cerr << "classification=missing_select_ast" << std::endl;
        return 3;
    }
